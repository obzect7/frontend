...
export default {
  ...
  methods: {
    ...
    copyCode () {
      ...
      this.copyConfig = `// 사용자 정의 구성, 참조 ./default/setting.config.js，사용자 정의해야 하는 속성은 여기에서 구성할 수 있습니다.
      module.exports = ${formatConfig(config)}
      `
      let clipboard = new Clipboard('#copyBtn')
      clipboard.on('success', () => {
        this.$message.success(`복사 성공, 파일 덮어쓰기 src/config/config.js 프로젝트를 다시 시작하여 적용하십시오.`).then(() => {
          const localConfig = localStorage.getItem(process.env.VUE_APP_SETTING_KEY)
          if (localConfig) {
            console.warn('PC에 저장된 테마 구성이 감지되었으며 구성 코드를 복사하여 적용하려면 먼저 구성을 재설정해야 할 수 있습니다.')
            this.$message.warn('PC에 저장된 테마 구성이 감지되었으며 구성 코드를 복사하여 적용하려면 먼저 구성을 재설정해야 할 수 있습니다.', 5)
          }
        })
        clipboard.destroy()
      })
    ...
    resetSetting() {
      this.$confirm({
        title: '테마를 재설정하면 페이지가 새로고침되며 현재 페이지 콘텐츠는 유지되지 않습니다. 재설정하시겠습니까?',
        onOk() {
