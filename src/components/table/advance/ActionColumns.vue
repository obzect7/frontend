<template>
  <div class="action-columns" ref="root">
      <a-popover v-model="visible" placement="bottomRight" trigger="click" :get-popup-container="() => $refs.root">
        <div slot="title">
          <a-checkbox :indeterminate="indeterminate" :checked="checkAll" @change="onCheckAllChange" class="check-all" />컬럼
        ...
        <a-list style="width: 100%" size="small" :key="i" v-for="(col, i) in columns" slot="content">
          <a-list-item>
            ...
            <template slot="actions">
              <a-tooltip title="컬럼헤드 고정" :mouseEnterDelay="0.5" :get-popup-container="() => $refs.root">
              ...
              <a-tooltip title="검색추가" :mouseEnterDelay="0.5" :get-popup-container="() => $refs.root">
